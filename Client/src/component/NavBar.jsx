    ...
    <div className="w-full justify-center items-center p-auto">
      <nav className="fixed w-[95%] left-[2.5%] top-[25px] z-50 flex items-center">
        <div className="container mx-auto flex justify-between items-center w-full h-full">
            ...
            <ul className="hidden md:flex space-x-6 text-lg text-gray-800 font-medium items-center ml-[50px] pl-4">
              {[
                ...
                <li key={name} className="relative">
                  <div className="flex items-center cursor-pointer hover:text-black" onClick={() => setDropdownOpen(dropdownOpen === index ? null : index)}>
                    <NavLink to={path} className={({ isActive }) => `cursor-pointer hover:text-black ${isActive ? "text-blue-600 font-semibold" : ""}`}>
                      {name}
                    </NavLink>
                    <FaChevronDown className="ml-1 text-sm" />
                  </div>
                  {dropdownOpen === index && (
                    <motion.ul initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="absolute left-0 mt-2 bg-white shadow-md rounded-md w-48">
                      {subItems.map((subItem, subIndex) => (
                        <li key={subIndex} className="px-4 py-2 hover:bg-gray-100">
                          <NavLink to={`${path}/${subItem.replace(/\s+/g, "")}`}>{subItem}</NavLink>
                        </li>
                      ))}
                    </motion.ul>
                  )}
          ...
          <button className="md:hidden text-gray-900 text-3xl leading-none" onClick={() => setIsOpen(!isOpen)}>
            {isOpen ? <FaTimes /> : <FaBars />}
          </button>
        </div>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="md:hidden absolute top-20 left-0 w-full bg-gray-100 shadow-lg rounded-xl">
            <ul className="flex flex-col text-lg text-gray-800 font-medium space-y-5 py-6">
              {[
                ...
                <NavLink key={name} to={path} className={({ isActive }) => `px-6 hover:text-black leading-none ${isActive ? "text-blue-600 font-semibold" : ""}`}>
                  {name}
                </NavLink>
              ))}
              <NavLink to="/login" className="px-6 border-t pt-3 leading-none ml-2">
                Login
              </NavLink>
              <NavLink to="/Signup" className="px-6 bg-blue-600 text-white py-3 rounded-lg w-[20%] text-center leading-none">
                Signup
              </NavLink>
            </ul>
          </motion.div>
